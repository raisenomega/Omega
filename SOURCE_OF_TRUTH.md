...
| DEBT-032 | Migración 00005 (Step 3 §7, 18 may 2026) crea tabla `client_plans` con `current_period_end = created_at + 30d` como **default temporal sin Stripe activo**. 2 gaps abiertos: (a) cuando Stripe se integre (Fase 3), webhook `customer.subscription.updated` debe actualizar `current_period_end` real + `stripe_subscription_id`; (b) **no hay trigger en `clients` INSERT** que auto-cree fila `client_plans` — clients nuevos creados vía UI quedan sin plan hasta que app layer lo inserte explícitamente (el componente `PlanStatusBar` muestra "Sin plan configurado" gracefully) | Fase 3 §3.x billing: (1) endpoint webhook Stripe upsert `client_plans` real; (2) trigger AFTER INSERT en `clients` que crea fila `client_plans` con default basic+30d (O en app layer durante flow de crear cliente — decisión de arquitectura). El componente `PlanStatusBar` ya está preparado: gracefully degradado si no hay plan | 6h | Medio (billing dependency) |

**Total deuda estimada: ~576h (~14 semanas full-time)** · DEBT-004 subsumido por DEBT-017 (–60h estimadas); DEBT-012 no suma (cerrada parcial 18 may 2026); DEBT-013 +16h; DEBT-014 +40h; DEBT-015 +20h; DEBT-016 +2h; DEBT-017 +80h; DEBT-018 +6h; DEBT-019 +6h; DEBT-020 +16h; DEBT-021 +2h; DEBT-022 0h (cerrada §2.6); DEBT-023 0h (cerrada model bump); DEBT-024 +12h; DEBT-025 +8h; DEBT-026 0h (cerrada CORS parser); DEBT-027 0h (cerrada extras-forbidden); DEBT-028 0h (cerrada os.environ shim + JWT migration); DEBT-029 +6h; DEBT-030 +8h; DEBT-031 +4h; DEBT-032 +6h.
