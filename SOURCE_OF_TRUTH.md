...
| DEBT-032 | **CERRADA PARCIAL (19 may 2026)**: Migración 00006 implementa trigger `AFTER INSERT ON auth.users` que auto-crea `clients` (`plan='adopcion'`) + `client_plans` (`current_period_end = now() + 7 days`) vía función `auto_provision_client_on_signup` con `SECURITY DEFINER` + `search_path` pinned. Guard contra reseller owners vía `resellers.owner_user_id`. Reseller default `omega-direct` creado con Ibrain como owner. Backfill aplicado a `cliente@omega.com` (los otros 2 auth.users excluidos: Ibrain por guard, `reseller@omega.com` por filtro email temporal). **Parte 2 (Stripe webhook) sigue pendiente** Fase 3 §3.x billing | Parte 2 abierta: webhook `customer.subscription.updated` debe actualizar `current_period_end` real + `stripe_subscription_id` en `client_plans` cuando Stripe se integre | 2h (parte Stripe) | Medio (billing dependency) |
...
**Total deuda estimada: ~636h (~16 semanas full-time)** · DEBT-004 subsumido por DEBT-017 (–60h estimadas); DEBT-012 no suma; DEBT-013 +16h; DEBT-014 +40h; DEBT-015 +20h; DEBT-016 +2h; DEBT-017 +80h; DEBT-018 +6h; DEBT-019 +6h; DEBT-020 +16h; DEBT-021 +2h; DEBT-022 0h; DEBT-023 0h; DEBT-024 +12h; DEBT-025 +8h; DEBT-026 0h; DEBT-027 0h; DEBT-028 0h; DEBT-029 +6h; DEBT-030 +8h; DEBT-031 +4h; DEBT-032 +2h (parte trigger cerrada 19 may, queda parte Stripe); DEBT-033 +40h; DEBT-034 +16h; DEBT-035 +8h.
