...
| DEBT-016 | I1 violators backend (estado tras §2.5 18 may 2026): **eliminados** `runway_agent.py` + `fal_video_agent.py` + `generate_video_fal.py` + handler/router refs · **swapped** `generate_image.py` (DALL-E rama solo) y `generate_video.py` (Runway→Veo); **pendientes** §2.6: `groq_agent.py` (1 import), `generate_image.py` (rama FAL Flux Kontext sigue, ver DEBT-022), `openai_service.py` (6 imports), `providers/{openai,groq,deepseek}_provider.py` (3 imports), `ai_providers.py` (1 import), `services/llm/router.py` (1 import) + 6 `# type: ignore` en `base_agent.py`/`providers/anthropic_provider.py` | Lift & shift §2.1 (18 may 2026): código Lovable usa OpenAI/Groq directos. Exception list en checks 1/10/2/10 como **grace period Fase 2**. Cerrar en §2.6 (eliminar OpenAI/Groq/DeepSeek imports) | ~4h restantes | Crítico (DDD I1) |
...
| DEBT-022 | `generate_image.py` aún tiene `import fal_client` + rama FAL Flux Kontext para modo **edit** (con `reference_images_b64`) intacta. Lovable usa FAL Flux para editar imágenes existentes; Nano Banana soporta refs vía `reference_images_b64` en `nano_banana_adapter.generate()` pero `generate_image_compat` no lo expone aún | §2.5 (18 may 2026) scope refinado a solo video. **Cerrar en §2.5b o §2.6**: (a) extender `_image_compat.generate_image_compat()` para aceptar `reference_images_b64`, (b) reemplazar rama FAL Flux en `generate_image.py` por llamada a `generate_image_compat(reference_images_b64=...)`, (c) drop `import fal_client` final | 4h | Crítico (DDD I1) |

**Total deuda estimada: ~550h (~13.5 semanas full-time)** · DEBT-004 subsumido por DEBT-017 (–60h estimadas); DEBT-012 no suma; DEBT-013 +16h; DEBT-014 +40h; DEBT-015 +20h; DEBT-016 +4h (post-§2.5, antes 16h); DEBT-017 +80h; DEBT-018 +6h; DEBT-019 +6h; DEBT-020 +16h; DEBT-021 +2h; DEBT-022 +4h.
