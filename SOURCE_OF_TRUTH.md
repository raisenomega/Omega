...
| DEBT-026 | `backend/app/config.py:75-80` — `@validator("backend_cors_origins", pre=True)` no parsea strings CSV (`a,b,c`) con `pydantic_settings 2.10`. Resultado: `SettingsError: error parsing value for field "backend_cors_origins" from source "EnvSettingsSource"` bloquea arranque uvicorn aun con `.env` correcto. Descubierto al validar DEBT-012 resolución (18 may 2026) | Migrar a `field_validator(..., mode="before")` (pydantic v2 API) o usar `json_schema_extra` con `BeforeValidator`. Alternativa rápida: aceptar el value como `str` en `.env` con formato JSON (`["a","b","c"]`) y dejar que pydantic parsee nativo | 2h | Bloqueante (uvicorn no arranca local) |

**Total deuda estimada: ~566h (~14 semanas full-time)** · DEBT-004 subsumido por DEBT-017 (–60h estimadas); DEBT-012 no suma (cerrada parcial 18 may 2026: stubs locales — restauración real cubierta por DEBT-006+010); DEBT-013 +16h; DEBT-014 +40h; DEBT-015 +20h; DEBT-016 +2h (post-§2.6, antes 4h · solo C2); DEBT-017 +80h; DEBT-018 +6h; DEBT-019 +6h; DEBT-020 +16h; DEBT-021 +2h; DEBT-022 0h (cerrada §2.6); DEBT-023 0h (cerrada model bump); DEBT-024 +12h; DEBT-025 +8h; DEBT-026 +2h.
