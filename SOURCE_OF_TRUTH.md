...
| DEBT-074 | **NUEVA (26 may · audit guardian DEBT-068 follow-up · 🟠)**: el patrón `safe_insert` (definido en 6 repos: content_lab_v3/content_v3/clients_v3/calendar_v3/aria + `intelligence/_intelligence_repository.py`) ejecuta `fn(*args)` con INSERTs/SELECTs Supabase **síncronos inline** en handlers async (~20+ call sites: behavioral_events, agent_memory, brand_voice_corpus, generated_content/draft, status changes, snapshots) → **bloquean el event loop** en cada write/read secundario. Mismo patrón que DEBT-068 (uploads) pero transversal en toda la capa de escritura · per-op chico (1 INSERT) pero pervasivo (cada generación/mensaje ARIA/onboarding dispara varios). | Hacer `safe_insert` async (`await asyncio.to_thread(fn, ...)`) en los 5 repos + `await` en los call sites · alternativa para telemetría best-effort fire-and-forget: `asyncio.create_task`. Transversal (~20 call sites · verificar contexto async c/u). | 4h | Alta (event loop bloqueado en hot path · transversal) |
