...
| DEBT-062 | **NUEVA (25 may · audit · A2 · 🟠)**: `ClientSocialAccounts.tsx:65-73` INSERT escribe `organization_id/account_url/followers_count/connected` — **4 columnas inexistentes** en `social_accounts` V3 (real: `approx_followers`, `oauth_status`, `platform_account_id`). "Agregar cuenta" da error · render lee `followers_count`/`connected` (siempre undefined). | Mapear INSERT/render a columnas reales (`approx_followers`, `oauth_status`, `status`) · quitar `organization_id`/`account_url`. | 2h | Alta (CRUD de cuentas roto) |
| DEBT-063 | **NUEVA (25 may · audit · A3 · 🟠 · comercial)**: cliente que pagó ARIA Premium ($12 · `clients.aria_level=4` en DB) ve "Modelo 3.0" + banner "Actualizar a 4.0" porque `useARIAChat.ts:10-12` `PLAN_TO_LEVEL` nunca devuelve 4 (deriva del plan, no del nivel real). Riesgo de doble cobro percibido. | El frontend debe leer el `aria_level` REAL del backend (`ARIAMessage.aria_level`) para header/banner, no derivarlo solo del plan. | 1h | Alta (UX post-pago · revenue) |
| DEBT-064 | **NUEVA (25 may · audit · A4 · 🟠)**: routers legacy colisionan en `/content-lab` — `main.py:153` monta content_lab_v3 y `:159` el legacy (mismo prefix). El legacy expone `/generate/text`, `/providers/` e importa `AIProviders` (multi-provider · roza I1). Frontend usa solo v3 → legacy = dead routes montadas. | Desmontar `content_lab.router` de main.py:159 (como DEBT-031 con calendar) tras verificar cero callers. | 1h | Alta (cleanup · superficie de ataque) |
| DEBT-065 | **NUEVA (25 may · audit · A5 · 🟠 · relacionada DEBT-033)**: Tab Agente de ClientDetail = dropdown vacío permanente (`teamMembers=[]` hardcoded) y `assignMutation` escribe `clients.assigned_to` — **columna inexistente** en V3 (`ClientDetail.tsx:60`). NO muestra el nivel ARIA del cliente (era lo esperado). | Rediseñar Tab Agente: mostrar nivel ARIA activo (clients.aria_level) + estado · eliminar lógica legacy team-member/`assigned_to`. | 2h | Alta (tab inútil) |
| DEBT-066 | **NUEVA (25 may · audit · M2 · 🟡)**: header de ClientDetail (`:108`) lee `client.email/company/phone` — **columnas inexistentes** en `clients` V3 (real: `business_email`/`website` · 00024). `.filter(Boolean)` evita crash pero el header solo muestra el nombre. | Usar `business_email`/`website` + campos del wizard (client_context) en el header. | 0.5h | Baja (display · sin crash) |
