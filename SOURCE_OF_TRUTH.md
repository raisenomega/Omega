...
| DEBT-061 | **NUEVA (25 may · audit · C5 · 🔴)**: `crisis_manager_agent.py` (legacy · registrado main.py:133) genera drafts pero **NO importa LIMITS_OMEGA ni valida `respond_to_complaint_publicly`** → no cumple verificable AS-R2/P4 "Crisis NUNCA publica solo" (no publica porque no puede, no por guardrail). `model="claude-opus-4"` inválido (`:34`). Frontend = ComingSoon (no lo consume hoy). | Si se activa: check de `ACCIONES_PROHIBIDAS` antes de output publicable + test que falle primero (G2) + corregir model ID. O dejar desregistrado hasta Sprint dedicado. | 3h | Crítica (regla inviolable P4 si se activa sin guard) |
