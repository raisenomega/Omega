...
| DEBT-057 | **NUEVA (25 may · audit cliente E2E · C1 · 🔴)**: Tab AI del cliente (`ClientAIConfig.tsx:22,31,41`) vende "Lovable AI integrado" + "hasta 3 proveedores" + `maxProviders:99` → **viola la regla NON-NEGOTIABLE Solo-Anthropic** (CLAUDE.md). UI legacy Lovable multi-proveedor que sobrevivió al reset V3. | Rediseñar la Tab AI a Anthropic-only (nivel ARIA + config real) O retirarla. Incluye el fix de DEBT-058. | 6h | Crítica (viola regla inviolable de marca/contrato) |
| DEBT-058 | **NUEVA (25 may · audit · C2 · 🔴)**: Tab AI consulta tablas FANTASMA `ai_providers` + `client_ai_config` (`ClientAIConfig.tsx:66-85`) — cero migraciones las crean → rota al cargar + upsert falla. | Se resuelve junto con el rediseño de DEBT-057 (Anthropic-only no necesita esas tablas). | 0h (subsumido por DEBT-057) | Crítica |
