...
| DEBT-059 | **NUEVA (25 may · audit · C3 · 🔴 · viola P1)**: el logo del wizard NUNCA se persiste. `SectionBrandAssets.tsx:22-28` recolecta `logo_files: File[]` pero `useOnboardingForm.ts:60-72` solo sube el doc de contexto · `onboarding-api.ts` cero refs a logo → el File se descarta. Consecuencia: el toggle "usar mi logo" del Content Lab es no-op (`find_client_logo_url` siempre None). El cliente sube su logo, la UI lo acepta, nunca aparece. | En `useOnboardingForm` onSuccess: subir `brand_assets.logo_files` vía `POST /clients/{id}/brand-files` (endpoint ya existe) → escribir `client_brand_assets.logo_file_id`. Cierra el overlay (= Logo Fase 2). | 5h | Crítica (P1: UI promete lo que no ocurre) |
