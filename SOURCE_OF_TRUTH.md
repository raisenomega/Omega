...
| DEBT-023 | `claude_service.model` hardcoded a `claude-sonnet-4-5-20250929` (Sonnet 4.5 legacy). CLAUDE.md V3 manda Haiku/Sonnet/Opus 4.6/4.7 con routing dinámico por agent_code. Tras §2.6 los 48 callers de claude_service usan ese modelo legacy fijo | Quick fix: bump constante en `claude_service.__init__` a `claude-sonnet-4-6`. Long-term cubierto por DEBT-024 (migrar a anthropic_adapter V3 con routing por agent_code) | 1h | Medio (model freshness) |
