-- ═══════════════════════════════════════════════════════════════════
-- SEED prompt_vault DIRECTO · 30 prompts production-tested
-- Para ejecutar manualmente en Supabase SQL Editor (NO via supabase db push)
--
-- USO:
--   1. Verificar tabla vacía:  SELECT COUNT(*) FROM prompt_vault;  → 0
--   2. Pegar TODO este archivo en SQL Editor + Run
--   3. Verificar:              SELECT COUNT(*) FROM prompt_vault;  → 30
--
-- Sin ON CONFLICT · INSERT directo · si hay rows previos puede trips el
-- unique index (category, vertical, platform, slot_name) · en ese caso
-- correr primero: DELETE FROM prompt_vault;
-- ═══════════════════════════════════════════════════════════════════
