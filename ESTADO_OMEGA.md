...
## 🔜 ARCO 2 · FUENTES WEB/SEO DIRECTO (GA4+GSC) — PUNTO DE CONTINUACIÓN (23 jun · próxima sesión arranca en Fase A)
...
- **Fase A — CÓDIGO (PRÓXIMA SESIÓN ARRANCA ACÁ):** `/google/authorize` toma el `client_id` del **Switcher** (query param + ownership check · patrón Zernio `/clients/{id}/...`) en vez de `find_client_for_user` (user). → cierra **DEBT-ANALYTICS-OAUTH-PER-CLIENT** (un reseller multi-negocio elige cuál conectar). + tests.
- **Fase B — CÓDIGO:** UI nueva en Cuentas Sociales — **SECCIÓN SEPARADA debajo de las redes sociales** con **LEYENDA (pedido explícito del owner):** ARRIBA = redes sociales (orgánico IG/FB/TikTok) · ABAJO = Meta/Google y demás para **ADS y ANALYTICS WEB** (qué conecta cada una, para qué, por qué). Botón "Conectar Google" estilo Zernio (usa `useGoogleOAuth`) + el consumo en el panel/chip Google.
