...
## 🚩 DEUDAS QUE UN CLIENTE REAL ENCONTRARÍA — RESOLVER PRE-LAUNCH (25 jun)

Estas tocan **producción real** (lo que vive un cliente externo), no son hardening interno ni cuentas de prueba. Resolver/decidir ANTES de lanzar a clientes reales.

1. **DEBT-GOOGLE-OAUTH-VERIFICATION** 🟠 — la app de Google OAuth está en **Testing mode**: solo conectan los hasta-100 usuarios agregados manualmente como "test users". **Un cliente real NO puede conectar su GA4/Search Console** hasta que se **publique la app** + pase la **verificación de Google** del scope sensible (`analytics.readonly`). NO es fix de código (trámite con Google · política de privacidad + ToS + dominio verificado + posible video + review de días/semanas). **BLOQUEA que clientes reales conecten analytics.**

2. **DEBT-ZERNIO-TIKTOK-TITLE-FIELD** 🟡 — B4 hace que TikTok-foto **PUBLIQUE** (ya no falla), pero el caption >90 se **RECORTA a 90** (TikTok usa el content como título del slideshow · sin body separado). El cliente publica **pero pierde texto** en TikTok foto. Cero-pérdida requiere confirmar con Zernio si acepta un campo `title` separado. **NO bloquea publicar · SÍ degrada el contenido del cliente** en TikTok foto.

3. **DEBT-ZERNIO-ORPHAN-RECOVERY** 🟡 — si un profile de Zernio se borra/pierde del lado de Zernio, OMEGA queda con **referencia colgante** y el cliente **NO se auto-recupera** (la idempotencia de Capa B solo re-crea si el `zernio_profile_id` es NULL). Hoy es raro (requiere borrado externo) · pero en producción dejaría a un cliente **colgado sin recuperación automática**.

> Las otras deudas nuevas del arco Zernio — **DEBT-ZERNIO-ERROR-STATUSCODE** 🟢 (hardening interno del mapeo de errores) y **DEBT-ZERNIO-ORPHAN-CLIENTE** 🟢 (cuenta de prueba cliente@) — **NO tocan a un cliente real en producción** · quedan en la lista general (`SOURCE_OF_TRUTH.md §6`), no en esta sección.
...
>
> ✅ **CONNECT EN VIVO COMPLETADO (25 jun · B2/B2.6 · Fase C/D del owner ejecutada):** el "funcionan al conectar" del 23 jun era a nivel **código/conexión** — en prod el connect estaba **roto** por 2 bugs de config que se cerraron el 25 jun (detalle técnico en `SOURCE_OF_TRUTH.md §6 · DEBT-OAUTH-REDIRECT-MISMATCH`): **(B2)** `redirect_uri_mismatch` — Google Cloud tenía el path con `auth`, el código sirve en `oauth` → agregado el URI correcto en Google Cloud (Railway estaba BIEN · el `.env` local mentía). **(B2.6)** `ValueError` en el callback — `OAUTH_ENCRYPTION_KEY` no era Fernet-válida → rotada a una válida (rotación limpia · `oauth_tokens` vacía). **Verificado en vivo:** `oauth_tokens` tiene 1 fila `provider=google` para Mail Boxes `7663aa55` (token cifrado + refresh + scopes analytics.readonly/webmasters). **2 deudas derivadas** (SOT §6): **B2.7** `/oauth/return` 404 (el relay del popup B3a redirige al dominio backend, vive en frontend · UX, token igual se guarda) + **DEBT-OAUTH-FERNET-HARDENING** (que `_fernet` capture el ValueError → 503 honesto). **Pre-launch:** **DEBT-GOOGLE-OAUTH-VERIFICATION** (app en modo Testing · publicar+verificar `analytics.readonly` para clientes reales). El gap "número real >0" (sitio con tráfico + tag GA4) sigue siendo pendiente del owner, NO deuda de código.
>
...
## 📦 ARCO MODAL-AGENDAR CONTENT LAB · A→F · CIERRE · 26 jun 2026 (verificado en vivo · HEAD `e94da66`)

Simplificación + multi-red del modal de bloque de Content Lab. 6 commits en prod, todos con gate 15/15 y sagrado intacto (limits `213e3c01` · personas NOVA `bef773c9` / ARIA `054a17f3`).

- **A+B** — `MIN_PIECES` 3→2 (regla real: 1 caption + 1 media · `textCount≥1 && mediaCount≥1`) + `MAX_PIECES=12`.
- **C** — miniatura `<img>`/`<video>` en las filas + modal agrandado (`max-w-3xl` + `max-h-[90vh]` scroll).
- **D** — ayuda explicativa en el footer + quitar `ICON_BY_TYPE`.
- **E `b8bb201` — FAN-OUT MULTI-RED.** Un bloque se agenda en N redes marcadas (1 bloque → N `scheduled_posts` · producto cruzado N items de texto × M redes). **Opción 2**: el dropdown del FormBar siembra la red default tildada, el modal expande con checkmarks de las redes conectadas; el dropdown NO se quita ni inhabilita. Picker (`NetworkPicker.tsx`) espeja Supervisado (`DraftEditForm` · `useConnectedNetworks` → solo `status='active'`). Backend (`_fanout.py` puro + handler bifurcado en `schedule_post.py`) espeja `maybe_schedule_on_approve`: invariante "omite red sin cuenta active · jamás `social_account_id` NULL" · `insert_scheduled_posts_bulk` atómico · helper `first_active_account_id_or_none` en `calendar_v3/_access.py` (gemelo no-lanzante · NO acopla `content_v3` · DDD). Cuenta = primera active por red (DEBT-CL-015 intacto en el path legacy single-red del `else`). DOBLE GUARDA: front oculta+filtra no-conectadas, backend omite sin-cuenta y devuelve 422 si ninguna resuelve. 4 tests (`test_fanout.py`): 3/3 redes → 3 rows · 1 sin-cuenta omitida → 2 rows · 0 resuelven → 0 rows · single-red retrocompat → 1 row. **VERIFICADO EN VIVO:** raisen@ 3/3 redes · reseller@ Omega Raisen (`5a323aa3`) 1 bloque → 5 redes (TikTok/FB/LinkedIn published · X programado sin-conectar = guarda correcta · IG falló por ratio, ver F).
- **F `e94da66` — AVISO DE RATIO IG.** `FeedRatioWarning.tsx` espeja el aviso inline de Supervisado: si el bloque tiene una imagen con ratio (w/h) ∉ [0.8, 1.91] y la red Instagram está marcada → muestra la caja amber "Esta imagen es vertical… el feed de IG la rechaza · desmarcá IG o las demás publican igual". Regla y umbrales = backend (`_media_guard.py` · `feed-aspect.ts`). Solo Instagram, solo imágenes (video → Reel/Story leniente). Ratio leído de `naturalWidth/Height` igual que Supervisado. **INFORMA, no bloquea** (decisión owner · el botón Agendar sigue habilitado). Originado por la prueba de E: imagen 9:16 (ratio 0.56) → 4 redes OK e IG rechazada por la guard de OMEGA (no Zernio · no bug del fan-out). **VERIFICADO EN VIVO:** el aviso aparece.

**Control de calidad del arco:** `ScheduleModalV2.tsx` quedó en 99 LOC (techo ratchet 100) extrayendo `NetworkPicker.tsx` (E) y `FeedRatioWarning.tsx` (F). tsc 0 · ratchet C4 sin crecimiento · 4 tests fanout verdes + 37 calendar/supervised sin regresión.
...
## 🖼️ ARCO REDISEÑO MEDIA / BIBLIOTECA DE MEDIOS · CIERRE · 30 jun 2026 (verificado en vivo · HEAD `f388d64`)

Rediseño UX de la Biblioteca de Medios (`/content` · tab Media). **SOLO frontend · CERO backend** — los 4 handlers (copiar URL · navegar a Content Lab · descargar blob · eliminar) son los mismos; solo cambiaron de contenedor + se agregó preview. 5 commits en prod, gate 15/15 c/u, sagrado intacto.

- **`b7ceb8c`** — extrae `MediaCard` + **thumbnail de video** (primer frame vía `<video>` con `#t` · 100% cliente · sin ffmpeg/backend).
- **`2f7f38a`** — las 4 acciones salen del overlay-hover a un botón **[Ver]** (primary) → `DropdownMenu`.
- **`96da685`** — [Ver] a la **franja inferior** de la tarjeta (junto al nombre/peso).
- **`7eb6c52`** — click en el **thumbnail** → **pop-up preview** (`Dialog` · imagen `object-contain` / video reproduce). Zonas separadas: thumbnail = preview · franja inferior = menú.
- **`f388d64`** — [Ver] → **ícono ⋮** (`MoreVertical` · "más opciones") + `aria-label`.

**Estado de la página:** cada tarjeta de Media muestra en la franja inferior un botón **⋮** (amarillo de marca) que abre el menú con **Copiar · Usar en Content Lab (solo imágenes) · Descargar · Eliminar**. Click en el thumbnail → preview grande. Thumbnail de video = primer frame por `<video>#t` (cero backend). **DEBT-MEDIA-RENAME** registrada (editar nombre diferido · storage.move cambia la URL).

## 💡 ARCO "LA IDEA ES LA UNIDAD" · CIERRE · 30 jun 2026 (verificado en vivo · HEAD `2417856`)

⚠️ **CAMBIO DE MODELO** de la página Estrategias: de **"estrategia-como-unidad"** a **"IDEA-como-unidad"**. Una estrategia tiene N ideas (`contenido.posts_sugeridos` · el índice del array es el handle estable · `contenido` write-once). **Usar / archivar / eliminar opera por IDEA, no por estrategia.** Una estrategia sale de Activas **solo cuando se usaron TODAS sus ideas** (flip a `used`).

**Modelo final (flujo):** Activas (estrategias · **sin botones** · borde amarillo · contador **"X de N disponibles"** = las que quedan) → **modal** (1 flecha por idea · las **usadas OCULTAS** por cruce `idx ∈ usedIdxs`) → la flecha **registra la idea** (`POST /use-idea` · NO consume · best-effort) y la manda al generador → **Usadas** (ideas: red + texto + "De: {título}" + **Re-usar + Archivar**) → **Archivadas** (ideas: **Eliminar** con `AlertDialog` confirm · una sola dirección · sin des-archivar).

**Linaje (todos gate 15/15 · sagrado intacto):**
- **Precursor UX:** `e949772` header · `a127d82` modal+leyendas · `d8951ed` cuadros por red · `87c358e` vista usadas.
- **CAPA 1 medición (subsumida):** `f45f85c` migr **00081** `last_used` · `8298d29` `POST /use` · `d98e630` front · `ac50ead` **Fase 1** (tarjeta usada + Archivar estrategia) · `19f3d56` **Fase 2** (`DELETE /strategies/{id}` → huérfano).
- **Idea-level F0→C.3:** `a13eee6` F0 · `381315f` A (migr **00082**) · `53de42f` B.1 · `a8084a2` B.2/B.3 · `120f7fe` C.1 · `9691d55` C.2-back · `84cf1d5` C.2-front (migr **00083**) · `b74aff1` C.3-back · `2417856` C.3-front.

**Estado de la página:** modelo idea-level **completo en prod**. 3 migraciones CEO-gated (00081/00082/00083 · la última aplicada manual por el owner). Ownership 404 sin fuga en `use-idea`/`used-ideas`/`archive`/`delete-idea` (scope `client_id` del servidor). Routing de las rutas literales `used-ideas/...` antes de `/{strategy_id}` (verificado por **openapi**, no `app.routes` · trampa starlette 1.x).

### Deudas del arco (registradas · NO atacadas · detalle en SOT §6)
- **DEBT-HUÉRFANOS-ESTRATEGIA** — `DELETE /strategies/{id}` (Fase 2) + Archivar-estrategia (Fase 1) sin UI por el modelo idea-level. Endpoints vivos pero inertes (NO bugs). Trigger: dejarlos o removerlos en limpieza.
- **DEBT-TEST-VS-REALIDAD** — los tests mockean los hooks (verifican que la flecha llama, no la escritura real). El bug del bundle viejo de Vercel lo destapó. Trigger: arcos con escritura real → verificación viva (SELECT read-only) + Vercel "Ready" + Ctrl+Shift+R.
