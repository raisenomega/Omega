...
- **DEBT-PLACEMENT** 🟡 — F informa que el 9:16 no entra en feed IG; el fix real (publicar el vertical como Reel/Story en vez de rechazarlo) es trabajo aparte · placement por-formato per-plataforma (`_media_guard.py:11`).
- **DEBT-CONTENTLAB-STORY-CAROUSEL-AGENDA** 🟡 — ya registrada (story/carousel en no-agenda provisional).

### Pendiente · próximo arco (NO en este)
- **CARRUSEL (N media en 1 post)** — arco distinto y ortogonal al fan-out (E = N REDES · carrusel = N MEDIA en 1 post). 3 capas a destapar (sonda previa): `useScheduleBlock.ts` `.find`→`filter` · `scheduled_posts.media_url` singular→array · `_publish_service` lista-de-1. **PREGUNTA MADRE:** Zernio renderiza carrusel — confirmado por DOC (`docs.zernio.com/guides/media-uploads`) · NO confirmado en vivo · lo confirma el owner publicando 2 URLs en cuenta de prueba. NO arranca hasta esa prueba.
