...
from app.bc_cognition.infrastructure import rex_live_repository as live
...
def select_publish_fn(client_id: str) -> PublishFn:
    ...
    is_live = live.rex_live_enabled() and live.reseller_rex_live(client_id)
    logger.info(f"rex publish_fn[{client_id}] = {'LIVE' if is_live else 'shadow'} (maestro+reseller)")
    return _live_publish if is_live else _shadow_publish
