"""Red de seguridad del adapter (generate · Capa 7-A). ai_provider_execute mockeado
(aísla el componente · cero API real). Patrón AsyncMock + asyncio.run. G9 exime tests."""
import asyncio
...
def _patch_execute(monkeypatch, execute_mock):
    """Aísla la ruta real de generate() (Capa 7-A): ai_provider_execute + record_mcp_use."""
...
def test_generate_returns_text_from_content(monkeypatch):
    execute = AsyncMock(return_value=_fake_message(text="respuesta"))
...
def test_generate_passes_cache_control_system_block(monkeypatch):
    ...
    kwargs = execute.call_args.args[0]
...
def test_generate_no_tools_by_default(monkeypatch):
    ...
    assert "tools" not in kwargs  # RETROCOMPAT 1b: sin tools= la clave NO se pasa
...
def test_generate_invalid_agent_code_returns_error(monkeypatch):
    ...
    execute.assert_not_called()  # no llega a la API si el agent_code es inválido
...
def test_generate_computes_usage_and_cache_hit(monkeypatch):
    execute = AsyncMock(return_value=_fake_message(in_tok=100, out_tok=20, cache_read=80))
    _patch_execute(monkeypatch, execute)
    resp, err = asyncio.run(ad.generate("aria_1", "sys", [{"role": "user", "content": "x"}]))
...
def test_generate_with_tools_passes_them(monkeypatch):  # 1b · plomería · ninguna tool real conectada
    ...
    assert execute.call_args.args[0]["tools"] == tools
