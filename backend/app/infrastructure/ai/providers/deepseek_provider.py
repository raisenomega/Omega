...
Deepseek V3 Chat and R1 Reasoner support.
...
class DeepseekProvider(BaseAIProvider):
    """
    Deepseek V3 and R1 provider.

    Models:
    - deepseek-chat: V3 general-purpose chat
    - deepseek-reasoner: R1 deep reasoning model

    Used by: LUNA, SHIELD, FORGE, DEBUG, SCOPE (Product/Tech)
    ...
    def _validate_model(self, model_name: str) -> None:
        ...
        if model_name not in valid_models:
            raise ValueError(
                f"Invalid Deepseek model: {model_name}. "
    ...
    async def chat(
        ...
        try:
            ...
            logger.info(
                f"Deepseek {self._model_name}: {tokens_used} tokens, "
            ...
            logger.error(
                f"Deepseek {self._model_name} error: {type(e).__name__}: {e}",
                exc_info=True
            )
            raise
