...
import { ClientDraftModal } from "@/components/clients/ClientDraftModal";
...
function DraftRow({ d, onApprove, onReject, onOpen, busy }: {
  d: SupervisedDraft; onApprove: () => void; onReject: () => void; onOpen: () => void; busy: boolean;
}) {
  // Clic en la tarjeta abre el detalle (modal) · los botones paran la propagación para no abrirlo.
  return (
    <div className="rounded-lg border border-border/40 bg-muted/10 p-3 space-y-2 cursor-pointer hover:bg-muted/20 transition-colors"
      onClick={onOpen}>
      ...
      <div className="flex gap-2 pt-1">
        <Button size="sm" className="gap-1 h-7" disabled={busy}
          onClick={(e) => { e.stopPropagation(); onApprove(); }}>
          <Check className="h-3.5 w-3.5" /> Aprobar
        </Button>
        <Button size="sm" variant="outline" className="gap-1 h-7" disabled={busy}
      ...
      <CardContent>
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
            {items.map((d) => (
              <DraftRow key={d.id} d={d} busy={busy} onOpen={() => setSelected(d)}
