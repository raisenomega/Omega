...
interface Props {
  ...
  onPendingFile?: (f: File | null) => void;  // FIX 1 · retiene doc para subir al crear
}
...
export function SectionBusiness({ form, clientId, onPendingFile }: Props) {
  ...
  const handleFile = (f: File | undefined) => {
    ...
    if (!clientId) {
      // FIX 1: Nuevo Cliente (sin clientId) → retener · se sube automáticamente al crear.
      onPendingFile?.(f);
      setUploaded({ filename: f.name, chars: 0 });
  ...
  return (
    <div className="space-y-3">
      <div className="border border-dashed border-border rounded-lg p-2 bg-muted/30 flex items-center justify-between gap-2">
        ...
        <Button size="sm" variant="outline" onClick={() => fileRef.current?.click()} disabled={upload.isPending} className="gap-1 shrink-0 h-8">
      ...
      {shown && (
        <p className="text-[10px] text-emerald-600 flex items-center gap-1">
          ...
          {shown.filename} · {shown.chars > 0 ? `${shown.chars.toLocaleString()} chars · ARIA lo usará permanentemente` : "se subirá al crear el cliente"}
          {uploaded && <button type="button" onClick={() => { setUploaded(null); onPendingFile?.(null); }} aria-label="Limpiar visual" className="ml-1"><X className="h-3 w-3" /></button>}
        </p>
      )}
      ...
      <div className="grid grid-cols-2 gap-2 items-end">
        ...
        <div className="space-y-1"><Label className="text-xs">Años operando</Label>
          <Input className="h-8" type="text" inputMode="numeric" value={v?.years_operating ?? ""} onChange={(e) => set("years_operating", parseLooseNumber(e.target.value))} />
        </div>
      </div>
