...
async function generateContent(body: { platform: string; content_type: string; topic: string; tone: string }): Promise<Result> {
  const { data: { session } } = await supabase.auth.getSession();
  const base = import.meta.env.VITE_API_URL ?? "http://localhost:8000/api/v1";
  const r = await fetch(`${base}/content-lab/generate`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${session?.access_token ?? ""}` }, body: JSON.stringify(body) });
...
export default function ContentLabPage() {
  ...
  const m = useMutation({
    ...
    onSuccess: (r) => { setResult(r); toast({ title: "Contenido generado" }); },
    ...
    <div className="container mx-auto max-w-3xl px-4 py-6 space-y-4">
      ...
      <Card><CardContent className="space-y-3 p-4">
        ...
        <div className="space-y-1"><Label className="text-xs">Tipo</Label>
          ...
          <p className="text-[10px] text-muted-foreground mt-1 flex items-center gap-1">Imagen y video <Badge variant="outline" className="h-4 text-[9px]">Próximamente</Badge></p></div>
        <div className="space-y-1"><Label className="text-xs">Tema</Label>
          <Textarea value={topic} onChange={(e) => setTopic(e.target.value)} rows={3} placeholder="¿De qué trata este post?" className="resize-none" /></div>
